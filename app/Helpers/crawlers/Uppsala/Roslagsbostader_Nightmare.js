function roslagsbostader_crawl(callback) {
    ...
    function start(callback) {
        parser.getRobots('http://www.roslagsbostader.se/robots.txt', function (err, crawlDelay, parser) {
            if (err) {
    ...
    function* run() {
        let url = 'http://marknad.roslagsbostader.se/#seekAreaMode=simple&search=y&page=1&syndicateNo=1&objectMainGroupNo=1&rent=0;15000&area=0;150&advertisement=-1&type=standard&take=1000000';
        let staticUrl = 'http://marknad.roslagsbostader.se/';
        let array = [];
        ...
        let links = yield nightmare
            .evaluate(function() {
                var links = document.querySelectorAll('#search-container > div > div > div.panel-body > div:nth-child(3) > div > div > a.btn.pull-right.btn-primary');
...
module.exports.roslagsbostader_crawl = roslagsbostader_crawl;
